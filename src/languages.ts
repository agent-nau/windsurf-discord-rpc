...
export const LANGUAGE_ASSETS: Record<string, string> = {
...
export const FILE_EXTENSION_ASSETS: Record<string, string> = {
  // Vite
...
export function getAssetFromFilename(filename: string): string | undefined {
  const lower = filename.toLowerCase();
  const basename = lower.split(/[\\/]/).pop() || '';

  // Check exact basename matches first
  if (FILE_EXTENSION_ASSETS[basename]) {
    return FILE_EXTENSION_ASSETS[basename];
  }

  // Check for patterns in filename
  if (basename.includes('vite.config')) return 'vitejs';
  if (basename.includes('tailwind.config')) return 'tailwindcss';
  if (basename.includes('supabase.config')) return 'supabase';
  if (basename.includes('supabase.toml')) return 'supabase';
  if (basename === 'dockerfile' || basename.includes('docker-compose') || basename.includes('compose.yml')) return 'docker';
  if (basename.includes('.env')) return 'bash';
  if (basename.includes('readme')) return 'markdown';
  if (basename.includes('license')) return 'markdown';
  if (basename.includes('changelog')) return 'markdown';
  if (basename.includes('contributing')) return 'markdown';
  if (basename.includes('tsconfig')) return 'typescript';
  if (basename.includes('jsconfig')) return 'javascript';
  if (basename.endsWith('.sql')) return 'sql';
  if (basename.endsWith('.prisma')) return 'sql';
  if (basename === 'schema.prisma') return 'sql';

  return undefined;
}

// Get asset key for a language ID
export function getLanguageAsset(languageId: string): string {
  return LANGUAGE_ASSETS[languageId.toLowerCase()] || LANGUAGE_ASSETS['default'];
}
